# ADR-0004 — Prepare the full TinyStories corpus, train on a Chinchilla-bounded slice of it
...
| Related | [`docs/17_DATASET_GUIDE.md`](../../docs/17_DATASET_GUIDE.md) · [`research/paper_notes/chinchilla.md`](../paper_notes/chinchilla.md) · [`SPEC.md` §3](../../SPEC.md) · [`datasets/SOURCES.md`](../../datasets/SOURCES.md) · backlog [#7](../OPTIMIZATION_BACKLOG.md) / [benchmark 005](../../benchmarks/005_mmap_dataset.md) |

> **Implementation status:** this ADR is **proposed, not accepted**, because the Phase-5
> code it governs does not exist in the repository yet — there is no
> `scripts/prepare_tinystories.py`, no `configs/model_v0_2.yaml`, no
> `configs/train_tinystories.yaml`, and `datasets/` contains only `README.md` and
> `SOURCES.md` (whose TinyStories rows are still `_tbd_`). It is written now so the
> corpus-scope decision is settled *before* the preparation script is built, which is what
> principle 3 asks for ("an ADR before or at the moment [decisions] are made"). Flip the
> status to `accepted` when the preparation script lands and `SOURCES.md` is filled in.
...
[benchmark 005](../../benchmarks/005_mmap_dataset.md) measured the memory-mapped loader
...
[benchmark 005](../../benchmarks/005_mmap_dataset.md). Had the corpus still had to be fully
